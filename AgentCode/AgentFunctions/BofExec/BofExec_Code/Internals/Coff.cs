    ...
    class Coff
    {
        ...
        public Coff(byte[] file_contents, IAT iat)
        {
            try
            {
                ...
                base_addr = NativeDeclarations.NtAllocateVirtualMemory(IntPtr.Zero, (uint)(total_pages * Environment.SystemPageSize), NativeDeclarations.MEM_RESERVE, NativeDeclarations.PAGE_EXECUTE_READWRITE);
                ...
                for (int i =0; i<this.section_headers.Count; i++ )
                {
                    ...
                    if (section_header.SizeOfRawData != 0)
                    {
                        ...
                        var addr = NativeDeclarations.NtAllocateVirtualMemory(IntPtr.Add(this.base_addr, num_pages * Environment.SystemPageSize), (uint)(section_pages * Environment.SystemPageSize), NativeDeclarations.MEM_COMMIT, NativeDeclarations.PAGE_EXECUTE_READWRITE);
        ...
        public void SetPermissions()
        {
            ...
            foreach (var perm in this.permissions)
            {
                ...
                NativeDeclarations.NtProtectVirtualMemory(perm.Addr, (UIntPtr)(perm.Size), page_permissions, out _);
            ...
            foreach (var symbol in this.symbols) 
            {
                ...
                else if (symbol_name == this.HelperPrefix + "argument_buffer")
                {
                    if (serialised_args.Length > 0)
                    {
                        ...
                        this.argument_buffer = NativeDeclarations.NtAllocateVirtualMemory(IntPtr.Zero, (uint)serialised_args.Length, NativeDeclarations.MEM_COMMIT, NativeDeclarations.PAGE_READWRITE);
        ...
        internal void Clear()
        {
            ...
            if (this.argument_buffer != IntPtr.Zero)
            {
                ...
                NativeDeclarations.NtFreeVirtualMemory(this.argument_buffer, 0, NativeDeclarations.MEM_RELEASE);
            }
            ...
            foreach (var perm in this.permissions)
            {
                NativeDeclarations.NtProtectVirtualMemory(perm.Addr, (UIntPtr)(perm.Size), NativeDeclarations.PAGE_READWRITE, out _);

            }
            // zero out memory
            NativeDeclarations.ZeroMemory(this.base_addr, (int)this.size);
            NativeDeclarations.NtFreeVirtualMemory(this.base_addr, 0, NativeDeclarations.MEM_RELEASE);
